...
from StudentManage import app, login, db
...
def Taodanhsach():
    ...
    return render_template('taodanhsachlop.html', classes = dao.get_lop())
...
def ThemHocSinh():
    ...
    if request.method.__eq__('POST'):
        ...
        if not substring.__eq__('@gmail.com'):
            ...
            return render_template('tiepnhan.html', err_msg=err_msg)
        try:
            birthdate = datetime.strptime(ngaysinh, '%Y-%m-%d') #Hàm datetime.strptime() chuyển đổi chuỗi ngaysinh thành một đối tượng ngày tháng
        except:
            err_msg = 'Bạn chưa nhập ngày sinh. Vui lòng thử lại!'
            return render_template('tiepnhan.html', err_msg=err_msg)
        #Kiểm tra độ tuổi
        if (app.config['nambatdau'] - birthdate.year) < app.config['mintuoi'] or (   #15
                app.config['nambatdau'] - birthdate.year) > app.config['maxtuoi']:   #20
            err_msg = 'Ngày sinh không hợp lệ. Vui lòng thử lại!'
            return render_template('tiepnhan.html', err_msg=err_msg)

        #Tạo 1 dtuong hs mới thêm vào csdl
