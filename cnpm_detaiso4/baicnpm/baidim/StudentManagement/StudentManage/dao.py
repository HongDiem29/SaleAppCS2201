...
def get_subject_report(subject_id, semester_id):
    ...
    data = db.session.query(
        ...
        func.sum(case(
            (Test.diemm >= 5, 1),  # Thay đổi từ danh sách sang đối số theo thứ tự
            else_=0
        )).label('so_luong_dat')
    ...
    for subject in subjects:
        ...
        for i in range(len(scores)):
            ...
            if score_15m and score_45m and score_final:
                ...
                x = int(score_15m[2])#lấy số lượng
                y = int(score_45m[2])
                z = int(score_final[2])
                ...
                if total_count > 0:
                    ...
                    scores[i]['count'] += 1 #Tăng số lượng môn học đã tính điểm cho học sinh đó lên 1.
